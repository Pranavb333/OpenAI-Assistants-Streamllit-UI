...
from util.metrics import log_message_length, time_and_log
...
def makeUserMessage(userInput: Dict[str, str] = {}) -> Event:
    ...
    if not userInput and ("userInput" in st.session_state):
        ...
        # Log length of user message when the input type is not a button
        log_message_length('userMessage', userInput)
...
if __name__ == "__main__":
    ...
    if st.session_state.messages[-1]["role"] != "assistant":
            ...
            with st.spinner("Thinking..."):
                ...
                for reply in botEvent.botReply:
                    if reply.type == BotMessageTypes.button:
                        ...
                        makeButtons(reply.payload, makeUserMessage)
                    if reply.type == BotMessageTypes.text:
                        # Log the length of the bot response
                        log_message_length('botResponse', reply.payload.text)
        ...
        logger.info(f"Total conversation length is {len(st.session_state.messages)} messages")
